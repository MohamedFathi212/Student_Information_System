		...
		<div class="col-md-6 col-md-offset-3">
			...
			<form action="UpdateRecord.php?update_id=<?php echo $update_id;?>" method="post" enctype="multipart/form-data">
				  <div class="form-group">
				     الرقم.:<input type="text" class="form-control" name="roll" value="<?php echo 
				      $Roll;?>" >
				  </div>
				  <div class="form-group">
				    
				   الاسم<input type="text" class="form-control" name="fullname" value="<?php echo 
				    $Name;?>" placeholder="full name" required>
				  </div>
				  <div class="form-group">
				     المدينه: <input type="text" class="form-control" name="city" value="<?php echo $City;?>"  required>
				  </div>
				  <div class="form-group">
				    
				   رقم الوالد:<input type="text" class="form-control" name="pphone" value="<?php echo $Pcontact;?>" required>
				  </div>
				  <div class="form-group">
				    
				   المستوى:<input type="number" class="form-control" name="standard" value="<?php echo $Standard;?>" required>
				  </div>
...
//update button
	if (isset($_POST['submit'])) {
		if (!empty($_POST['roll']) && !empty($_POST['fullname'])) {
			...
			if ($Execute) {
				 $_SESSION['SuccessMessage'] = " تم التحديث بنجاح";
